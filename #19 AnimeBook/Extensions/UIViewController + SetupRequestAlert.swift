...
extension UIViewController {
    ...
    func showSetupRequestAlert(completion: @escaping ([ParamRequest]) -> ()) {
        ...
        setupAlertContent(textField: limitTF, startDatePicker: startDatePicker, endDatePicker: endDatePicker, alert: alert)
        
        let cancelAction = UIAlertAction(title: Constants.getLocalizableText("Cancel"),
                                         style: .cancel)
        let okAction = UIAlertAction(title: "OK",
                                     style: .default) { _ in
            var parameters: [ParamRequest] = []
            
            if let text = limitTF.text,
                let param = Int(text)  {
                parameters.append(.limit(param))
            }
            parameters.append(.startDate(startDatePicker.date))
            parameters.append(.endDate(endDatePicker.date))
    ...
    private func setupAlertContent(textField: UITextField,
        ...
        let titleLabel: UILabel = {
            let label = UILabel()
            label.text = Constants.getLocalizableText("Request Settings")
        ...
        ["Quantity", "Start Date", "End Date"].forEach {
            let label = UILabel()
            let text = Constants.getLocalizableText($0)
            label.text = text
