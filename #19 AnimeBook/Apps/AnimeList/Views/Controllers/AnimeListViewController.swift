...
final class AnimeListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        presenter.getAnime(with: [.limit(7), .letter("Bleach")])
    }
    
    // MARK: - Private func
    /// Настройка UI
    private func setupUI() {
        title = Constants.getLocalizableText("AnimeBook")
        setupSearchBar()
        setupNavBar()
        setupCollectionView()
    }
    ...
    @objc private func rightBarButtonPressed() {
        showSetupRequestAlert { parameters in
            self.presenter.getAnime(with: parameters)
        }
    }
}
...
extension AnimeListViewController: UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 0.5,
            ...
            self?.presenter.getAnime(with: [.letter("\(searchText)")])
        }
    }
}
